...
For `d_model <= 512` the single-GPU CLIP and LiT backwards instead use a FlashAttention-shaped kernel (`clip_fa_grad_kernel` in `clip_loss.py`): each program owns a block of gradient rows, streams the opposite tower, and accumulates in a `(64, next_pow2(d_model))` fp32 register tile written out exactly once — no gradient atomics at all. Each direction recomputes its similarities (the FlashAttention-2 dQ vs dK/dV trade), which is still far cheaper than the atomic read-modify-write traffic it replaces: measured ~2.8x end-to-end on A100 (B=65536, D=384, bf16, fwd+bwd: 512 -> 185 ms; B=131072: 2071 -> 703 ms; LiT B=65536: 103 ms). Larger `d_model` falls back to the atomic tile-grid kernel.
