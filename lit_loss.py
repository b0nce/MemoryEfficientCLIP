...
class StableMemoryEfficientLiTLossNormed(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x_normalized, y_normalized, temperature=0.07):
        """Forward pass for LiT loss."""
        X = x_normalized
...
class StableMemoryEfficientLiTLoss(nn.Module):
    """
    ...
    behavior of LiT, use lr / sqrt(batch_sz * temperature), where lr is the 
    learning rate for default LiT loss. Though for larger batch sizes, it worked
    best for me to just use some standard values like 1e-4, ignoring proper 
    scaling to mimic default loss behavior.
    """
    def __init__(self, temperature=0.07, normalized_inputs=False):
        super().__init__()
        self.temperature = temperature
    ...
    def forward(self, text_features, image_features):
        ...
        loss = StableMemoryEfficientLiTLoss.apply(text_features, image_features, self.temperature)
