...
def reference_loss(q, d, h, tau=TAU, margin=MARGIN, use_qq=False, use_dd=False,
    ...
    if tau_plus:   # debiased denominator (arXiv 2007.00224) over the nominal count
...
def run_single_clip():
    configs = [  # (B, K, use_qq, use_dd, stable, tau_plus)
        (1024, 0, False, False, False, 0.0),
        (1024, 4, False, False, False, 0.0),
        (1024, 4, True, False, False, 0.0),
        (1024, 4, False, True, False, 0.0),
        (1024, 4, True, True, False, 0.0),
        (1000, 3, True, True, False, 0.0),   # batch not a multiple of the block sizes
        (1024, 4, True, True, True, 0.0),    # stable gradient rescaling
        (1024, 4, True, True, False, 0.3),   # debiased contrastive loss
        (1024, 0, False, False, False, 0.5), # ... with the estimator clamp firing
    ]
    for dtype, tol in ((torch.float32, 1e-3), (torch.bfloat16, 5e-2)):
        for B, K, use_qq, use_dd, stable, tau_plus in configs:
            q, d, h = make_inputs(B, K, dtype, pos_dupes=tau_plus > 0)
            ...
            print(f"clip {str(dtype).split('.')[-1]} B={B} K={K} "
                  f"qq={use_qq} dd={use_dd} stable={stable} tau+={tau_plus} "
...
def run_single_lit():
    configs = [  # (B, K, use_qq, tau_plus)
        (1024, 0, False, 0.0),
        (1024, 4, False, 0.0),
        (1024, 4, True, 0.0),
        (1000, 3, True, 0.0),
        (1024, 4, True, 0.3),   # debiased contrastive loss
    ]
    for dtype, tol in ((torch.float32, 1e-3), (torch.bfloat16, 5e-2)):
        for B, K, use_qq, tau_plus in configs:
            q, d, h = make_inputs(B, K, dtype, pos_dupes=tau_plus > 0)
            ...
            ref = reference_loss(qr, d.float(), h.float() if h is not None else None,
                                 use_qq=use_qq, tau_plus=tau_plus)
            ref.backward()

            loss_fn = MemoryEfficientLiTQwen3Loss(
                temperature=TAU, margin=MARGIN, use_qq_negatives=use_qq,
                normalized_inputs=True, tau_plus=tau_plus)
            qk = q.detach().requires_grad_(True)
            loss = loss_fn(qk, d, h)
            loss.backward()

            print(f"lit {str(dtype).split('.')[-1]} B={B} K={K} qq={use_qq} tau+={tau_plus} "
...
def run_distributed():
    ...
    for K, use_qq, use_dd, tau_plus in [
            (0, False, False, 0.0), (4, False, False, 0.0), (4, True, False, 0.0),
            (4, False, True, 0.0), (4, True, True, 0.0), (4, True, True, 0.3)]:
        # identical global batch on every rank, each takes its shard
        q, d, h = make_inputs(B, K, torch.float32, seed=7, pos_dupes=tau_plus > 0)
        bad_q, bad_d = ambiguous_rows(q, d, h, use_qq, use_dd)
        kq, kd = ~bad_q[rows], ~bad_d[rows]
        qr, dr, hr = leafs(q, d, h)
        ref = reference_loss(qr, dr, hr, use_qq=use_qq, use_dd=use_dd, tau_plus=tau_plus)
        ref.backward()

        loss_fn = DistributedMemoryEfficientQwen3Loss(
            temperature=TAU, margin=MARGIN, use_qq_negatives=use_qq,
            use_dd_negatives=use_dd, normalized_inputs=True, tau_plus=tau_plus)
        ...
        if rank == 0:
            print(f"clip dist world={world} K={K} qq={use_qq} dd={use_dd} tau+={tau_plus} "
                  f"amb={int(bad_q.sum())}q/{int(bad_d.sum())}d")
    ...
    for K, use_qq, tau_plus in [(0, False, 0.0), (4, False, 0.0), (4, True, 0.0),
                                (4, True, 0.3)]:
        q, d, h = make_inputs(B, K, torch.float32, seed=11, pos_dupes=tau_plus > 0)
        bad_q, _ = ambiguous_rows(q, d, h, use_qq)
        kq = ~bad_q[rows]
        ...
        if rank == 0:
            print(f"lit dist world={world} K={K} qq={use_qq} tau+={tau_plus} "
                  f"amb={int(bad_q.sum())}q")
        check(f"loss(rank{rank})", total, ref.detach(), tol)
        check(f"dQ(rank{rank})", qs.grad[kq], qr.grad[rows][kq], tol)
        dist.barrier()
